...
public class LTTA {
	...
	public static void checkGbookGiven(EntityJoinWorldEvent event) {
		...
		if (entity instanceof EntityPlayer && !entity.getEntityWorld().isRemote
				&& !entity.getTags().contains(bookPlayerTag) && ModConfig.giveBookOnFirstSpawn == true) {
	...
	public static void onLootTableLoad(LootTableLoadEvent event) {
		if (event.getName().toString().equals("mystcraft:mystcraft_treasure")) {
			LootEntry customEntry = event.getLootTableManager()
					.getLootTableFromLocation(new ResourceLocation("ltta", "loot_narayan_book")).getPool("main")
					.getEntry("narayani_mystcraft");
